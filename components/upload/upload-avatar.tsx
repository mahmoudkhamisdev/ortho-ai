...
interface UploadAvatarProps {
  onChange?: (url: Blob | null) => void;
  value?: string | null;
  className?: any
}

export default function UploadAvatar({ onChange, value, className }: UploadAvatarProps) {
  const [
    { files, isDragging },
    {
      handleDragEnter,
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="relative inline-flex">
        ...
        <button
          type="button"
          className={cn("relative flex size-16 items-center justify-center overflow-hidden rounded-full border border-dashed border-input transition-colors outline-none hover:bg-accent/50 focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 has-disabled:pointer-events-none has-disabled:opacity-50 has-[img]:border-none data-[dragging=true]:bg-accent/50 hover:border-main duration-200", isDragging && "border-main", className)}
        ...
        >
          {finalImageUrl ? (
            <img
              ...
              src={finalImageUrl?.includes("/images") ? `${UrlImage}${finalImageUrl}` : finalImageUrl}
              alt="User avatar"
              width={64}
              height={64}
